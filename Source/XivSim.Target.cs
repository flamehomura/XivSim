...
public class XivSimTarget : TargetRules
{
	public XivSimTarget(TargetInfo Target) : base(Target)
	{
		Type = TargetType.Game;
		DefaultBuildSettings = BuildSettingsVersion.V2;
		ExtraModuleNames.Add("XivSim");
		CppStandard = CppStandardVersion.Cpp20;
	}
	
	private static bool bHasWarnedAboutShared = false;
	internal static void ApplySharedXivSimSettings(TargetRules Target)
	{
		...
		if (Target.BuildEnvironment == TargetBuildEnvironment.Unique)
		{
			...
			XivSimTarget.ConfigureGameFeaturePlugins(Target);
		}
		else
		{
			// !!!!!!!!!!!! WARNING !!!!!!!!!!!!!
